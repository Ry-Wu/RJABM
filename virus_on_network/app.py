...
from virus_on_network.model import State, VirusOnNetwork, number_infected, HospitalAgent


def agent_portrayal(graph):

    def get_agent(node):
        print(graph.nodes[node]["agent"][0])
    ...
    for u, v in graph.edges():
        agent1 = get_agent(u)
        agent2 = get_agent(v)
        if not isinstance(agent1, HospitalAgent) and not isinstance(agent1, HospitalAgent):
            w = 2
            ec = "#e8e8e8"
    ...
    for node in graph.nodes():
        ...
        if isinstance(agent, HospitalAgent):
            color = "tab:blue"  # Color for HospitalAgent
        else:
            node_color_dict = {
